...
export default function OrderHistoryModal({ isOpen, onClose, orders, onViewReceipt }: OrderHistoryModalProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="bg-white w-full max-w-lg rounded-[32px] overflow-hidden shadow-2xl flex flex-col max-h-[85vh] relative z-10 border border-slate-100"
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/50 no-scrollbar">
              {orders.length === 0 ? (
                ...
                orders.map((order) => (
                  <motion.div
                    key={order.id}
                    className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm space-y-4 hover:shadow-md transition-shadow relative overflow-hidden group"
                  >
                    {/* Top Order Meta */}
                    ...
                    <button
                      ...
                      className="w-full mt-2 bg-gradient-to-r from-orange-500/10 to-amber-500/10 hover:from-orange-500/15 hover:to-amber-500/15 text-[#FF6B00] py-3.5 rounded-2xl font-black text-xs uppercase tracking-wider transition-all active:scale-95 flex items-center justify-center gap-2 border border-orange-500/10"
                    >
                      Lihat Struk & Ulasan
                    </button>
                  </motion.div>
