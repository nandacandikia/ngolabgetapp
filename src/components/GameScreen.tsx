...
export default function GameScreen({ onClose, onGameComplete, userId }: GameScreenProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/90 backdrop-blur-md p-4">
      <motion.div
        ...
        className="w-full max-w-4xl h-[90vh] bg-white rounded-[32px] overflow-hidden flex flex-col relative shadow-2xl"
      >
        ...
        <div className="flex-1 relative bg-slate-100 flex flex-col items-center justify-center">
          ...
          {isPlaying && (
            <div className="z-10 text-center space-y-6 max-w-md p-8 bg-white/80 backdrop-blur-md rounded-3xl shadow-xl border border-white">
              ...
              <button
                onClick={simulateWin}
                className="w-full bg-gradient-to-r from-[#FF6B00] to-orange-500 text-white font-black py-4 rounded-2xl shadow-xl hover:shadow-orange-500/30 active:scale-95 transition-all flex justify-center items-center gap-2"
              >
                <Award size={20} />
